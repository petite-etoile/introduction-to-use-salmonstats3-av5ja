...
let state = Randomstring.generate(64);
let verifier = Randomstring.generate(64);
/**
 * OAuth認証用のURLを返す
 */
export const oauthURL = (() => {
    ...
    const baseURL = new URL('https://accounts.nintendo.com/connect/1.0.0/authorize');
 ...
 * 最終プレイ時間をリセットする
 */
export async function authorize(state, code) {
    if (state !== state)
        throw new Error('Provided state does not match.');
...
async function refresh_from_token(session_token, last_play_time) {
    console.log('Refreshing from token...', session_token, last_play_time);
    const hash = (await request(new Web.Hash.Request())).js;
    console.log('Hash', hash);
    const version = (await request(new NSO.Version.Request())).result;
    ...
    await keychain.set(plainToInstance(UserInfo, {
...
async function get_session_token(code, verifier) {
    return (await request(new SessionToken.Request(code, verifier))).session_token;
}
async function get_access_token(session_token) {
    return (await request(new AccessToken.Request(session_token))).access_token;
}
async function get_user_me(access_token) {
    return await request(new UserMe.Request(access_token));
}
async function get_coral_token(access_token, id, version) {
...
async function get_game_service_token(access_token, hash, version, user) {
    return (await request(new GameServiceToken.Request(access_token, hash, version, user)));
}
async function get_game_web_token(access_token, hash, version) {
    return (await request(new GameWebToken.Request(access_token, hash, version))).access_token;
}
async function get_bullet_token(access_token, version) {
    return (await request(new BulletToken.Request(access_token, version))).bullet_token;
}
export async function get_user_info() {
    return await keychain.get();
}
export async function set_user_info(last_play_time) {
